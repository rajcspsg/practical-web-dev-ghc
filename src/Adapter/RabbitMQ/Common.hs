...
consumeAndProcess :: (KatipContext m, FromJSON a, MonadUnliftIO m, Control.Monad.Catch.MonadCatch m) 
        ...
        Right payload -> do
            ...
            case result of
                Left err -> withMsgAndErr msg (displayException err) $ do
                    $(logTM) ErrorS "There was an exception when processing message"
                    return False
                Right bool -> return False
            
withMsgAndErr :: (KatipContext m, ToJSON e) => Message -> e -> m a -> m a
withMsgAndErr msg err =
    katipAddContext (sl "mqMsg" (show msg) <> sl "err" err)